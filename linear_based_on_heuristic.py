'''###########################################
CS221 Final Project: Heuristic Linear Q-Learning Implementation
Authors:
...
def modFeatureExtractor(state, action):
    ...
    # Feature based on heuristic
    angle_targ = state[0]*0.5 + state[2]*1.0        # angle should point towards center (s[0] is horizontal coordinate, s[2] hor speed)
    ...
    features = []
    # angle_targ
    features.append((('angle1', hover_todo > np.abs(angle_todo) and hover_todo > 0.05, action),1))
    # hover_targ
